 ...
 *
 * For a given connector state (i.e., Broadcast RGB property) and a given mode,
 * determine which RGB quantization range should be used.
 *
 * Returns:
...
{
	switch (state->broadcast_rgb) {
	default:
		WARN_ON(1);
		/* fallthrough */
	case DRM_MODE_BROADCAST_RGB_AUTOMATIC:
		return drm_default_rgb_quant_range(mode);
	case DRM_MODE_BROADCAST_RGB_FULL:
		return HDMI_QUANTIZATION_RANGE_FULL;
	case DRM_MODE_BROADCAST_RGB_LIMITED:
